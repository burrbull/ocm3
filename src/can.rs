...
trait CanExt {
    ...
    fn init(
        ...
        ts1: CanTimeSegment1,
        ts2: CanTimeSegment2,
        brp: u32, /*, loopback: bool, silent: bool*/
    ) -> isize;
...
impl CanExt for CAN {
    ...
    fn init(
        ...
        self.mcr.modify(|_, w| w.sleep().clear_bit());

        /* Request initialization "enter". */
        self.mcr.modify(|_, w| w.inrq().set_bit());

        /* Wait for acknowledge. */
        for _ in 0..CAN_MSR_INAK_TIMEOUT {
            if self.msr.read().inak().bit_is_set() {
                break;
            }
        }

        /* Check the acknowledge. */
        if self.msr.read().inak().bit_is_clear() {
            return 1;
        }
        ...
        self.mcr.modify(|_, w| w.inrq().clear_bit());
    ...
    fn filter_init(
        ...
        /* Request initialization "enter". */
        self.fmr.modify(|_, w| w.finit().set_bit());

        /* Deactivate the filter. */
        self.fa1r
            .modify(|r, w| unsafe { w.bits(r.bits() & !filter_select_bit) });

        self.fs1r.modify(|r, w| unsafe {
            w.bits(if scale_32bit {
        ...
        self.fm1r.modify(|r, w| unsafe {
            w.bits(if id_list_mode {
        ...
        self.fb[nr].fr1.write(|w| unsafe { w.bits(fr1) });
        /* Set the second filter register. */
        self.fb[nr].fr2.write(|w| unsafe { w.bits(fr2) });

        /* Select FIFO0 or FIFO1 as filter assignement. */
        self.ffa1r.modify(|r, w| unsafe {
            w.bits(if fifo != 0 {
                /* FIFO1 */
                r.bits() | filter_select_bit
            } else {
        ...
        if enable {
            self.fa1r.modify(|r, w| unsafe {
                w.bits(r.bits() | filter_select_bit) /* Activate filter. */
            });
        }

        /* Request initialization "leave". */
        self.fmr.modify(|_, w| w.finit().clear_bit());
    }
    ...
    #[inline]
    fn enable_irq(&mut self, irq: u32) {
        self.ier.modify(|r, w| unsafe { w.bits(r.bits() | irq) });
    }
    ...
    fn transmit(&mut self, id: u32, ext: bool, rtr: bool, length: u8, data: &[u8]) -> isize {
        ...
        tx.tir.modify(|_, w| unsafe {
            if ext {
                w.exid().bits(id) /* Set extended ID. */
            } else {
                w.stid().bits(id as u16)
            } /* Set standard ID. */
        });
        ...
        tx.tdtr.modify(|_, w| unsafe { w.dlc().bits(length) });
        /* Set the data. */
    ...
    #[inline]
    fn fifo_release(&mut self, fifo: usize) {
        self.rfr[fifo].modify(|_, w| w.rfom().set_bit());
    }
    ...
    fn receive(
        ...
        /* Get extended CAN ID. */
        let id = if ext {
            rx.rir.read().exid().bits()
        /* Get standard CAN ID. */
        } else {
            rx.rir.read().stid().bits() as u32
        };
        ...
        /* Get data. */
        rdxr = EightBytes {
            data32: [rx.rdlr.read().bits(), rx.rdhr.read().bits()],
        };
