...
macro_rules! impl_i2c {
    ($I2Cx:ty) => {
        impl I2cExt for $I2Cx {
            ...
            fn clear_stop(&mut self) {
                self.cr1.modify(|_, w| w.stop().clear_bit());
            }

            fn set_own_7bit_slave_address(&mut self, slave: u8) {
                /* Datasheet: always keep 1 by software. */
                self.oar1.modify(|r, w| unsafe {
                    w //????
                        .bits(r.bits() | (1 << 14))
                        .add7()
                        .bits(slave)
            ...
            fn set_own_7bit_slave_address_two(&mut self, slave: u8) {
                self.oar2.modify(|_, w| unsafe { w.add2().bits(slave) });
            }

            fn enable_dual_addressing_mode(&mut self) {
                self.oar2.modify(|_, w| w.endual().set_bit());
            }

            fn disable_dual_addressing_mode(&mut self) {
                self.oar2.modify(|_, w| w.endual().clear_bit());
            }

            fn set_clock_frequency(&mut self, freq: u8) {
                self.cr2.modify(|_, w| unsafe { w.freq().bits(freq) });
            }

            fn send_data(&mut self, data: u8) {
                self.cr2.modify(|_, w| unsafe { w.freq().bits(data) });
            }

            fn set_fast_mode(&mut self) {
                self.ccr.modify(|_, w| w.f_s().set_bit());
            }

            fn set_standard_mode(&mut self) {
                self.ccr.modify(|_, w| w.f_s().clear_bit());
            }

            fn set_ccr(&mut self, freq: u16) {
                self.ccr.modify(|_, w| unsafe { w.ccr().bits(freq) });
            }

            fn set_trise(&mut self, trise: u8) {
                self.trise.write(|w| unsafe { w.trise().bits(trise) });
            }

            fn send_7bit_address(&mut self, slave: u8, readwrite: RW) {
                //let bits = ((slave as u16) << 1) as u8 ) | ( readwrite as u8)
                let bits = (slave << 1) | (readwrite as u8);
                self.dr.write(|w| unsafe { w.dr().bits(bits) });
            }
            ...
            fn enable_interrupt(&mut self, interrupt: u32) {
                self.cr2
                    .modify(|r, w| unsafe { w.bits(r.bits() | interrupt) });
            }
            ...
            fn nack_next(&mut self) {
                self.cr1.modify(|_, w| w.pos().set_bit());
            }

            fn nack_current(&mut self) {
                self.cr1.modify(|_, w| w.pos().clear_bit());
            }

            fn set_dutycycle(&mut self, dutycycle: DutyCycle) {
                self.ccr.modify(|_, w| w.duty().bit((dutycycle as u8) == 1));
            }

            fn enable_dma(&mut self) {
                self.cr2.modify(|_, w| w.dmaen().set_bit());
            }

            fn disable_dma(&mut self) {
                self.cr2.modify(|_, w| w.dmaen().clear_bit());
            }

            fn set_dma_last_transfer(&mut self) {
                self.cr2.modify(|_, w| w.last().set_bit());
            }

            fn clear_dma_last_transfer(&mut self) {
                self.cr2.modify(|_, w| w.last().clear_bit());
            }
        }
