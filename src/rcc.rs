...
use crate::flash::{FlashExt, FlashLatency};
...
impl RccF1Ext for RCC {
    fn osc_ready_int_clear(&mut self, osc: Osc) {
        ...
        match osc {
            Osc::PLL => {
                reg.modify(|_, w| w.pllrdyc().set_bit());
            }
            /*
            ...
            */
            Osc::HSE => {
                reg.modify(|_, w| w.hserdyc().set_bit());
            }
            Osc::HSI => {
                reg.modify(|_, w| w.hsirdyc().set_bit());
            }
            Osc::LSE => {
                reg.modify(|_, w| w.lserdyc().set_bit());
            }
            Osc::LSI => {
                reg.modify(|_, w| w.lsirdyc().set_bit());
            }
        }
    }
    ...
    fn osc_ready_int_enable(&mut self, osc: Osc) {
        ...
        match osc {
            Osc::PLL => {
                reg.modify(|_, w| w.pllrdyie().set_bit());
            }
            /*
            Osc::PLL2 => { reg.modify(|_,w| w
                    .pll2rdyie) .set_bit() );
            ...
            */
            Osc::HSE => {
                reg.modify(|_, w| w.hserdyie().set_bit());
            }
            Osc::HSI => {
                reg.modify(|_, w| w.hsirdyie().set_bit());
            }
            Osc::LSE => {
                reg.modify(|_, w| w.lserdyie().set_bit());
            }
            Osc::LSI => {
                reg.modify(|_, w| w.lsirdyie().set_bit());
            }
        }
    }
    ...
    fn osc_ready_int_disable(&mut self, osc: Osc) {
        ...
        match osc {
            Osc::PLL => {
                reg.modify(|_, w| w.pllrdyie().clear_bit());
            }
            /*
            Osc::PLL2 => { reg.modify(|_,w| w
                    .pll2rdyie) .clear_bit() );
            ...
            */
            Osc::HSE => {
                reg.modify(|_, w| w.hserdyie().clear_bit());
            }
            Osc::HSI => {
                reg.modify(|_, w| w.hsirdyie().clear_bit());
            }
            Osc::LSE => {
                reg.modify(|_, w| w.lserdyie().clear_bit());
            }
            Osc::LSI => {
                reg.modify(|_, w| w.lsirdyie().clear_bit());
            }
        }
    }
    ...
    fn css_int_clear(&mut self) {
        self.cir.modify(|_, w| w.cssc().set_bit());
    }
    ...
    fn osc_off(&mut self, osc: Osc) {
        match osc {
            ...
            */
            Osc::HSE => {
                self.cr.modify(|_, w| w.hseon().clear_bit());
            }
            Osc::HSI => {
                self.cr.modify(|_, w| w.hsion().clear_bit());
            }
            Osc::LSE => {
                self.bdcr.modify(|_, w| w.lseon().clear_bit());
            }
            Osc::LSI => {
                self.csr.modify(|_, w| w.lsion().clear_bit());
            }
        }
    }
    ...
    fn css_disable(&mut self) {
        self.cr.modify(|_, w| w.csson().clear_bit());
    }

    fn set_sysclk_source(&mut self, clk: SysClk) {
        self.cfgr.modify(|_, w| w.sw().variant(clk));
    }

    fn set_pll_multiplication_factor(&mut self, mul: PllMul) {
        self.cfgr.modify(|_, w| w.pllmul().variant(mul));
    }

    fn set_pll_source(&mut self, pllsrc: PllSrc) {
        self.cfgr.modify(|_, w| w.pllsrc().variant(pllsrc));
    }

    fn set_pllxtpre(&mut self, pllxtpre: PllXtPre) {
        self.cfgr.modify(|_, w| w.pllxtpre().variant(pllxtpre));
    }
    ...
    fn set_rtc_clock_source(&mut self, clock_source: Osc) {
        match clock_source {
            Osc::LSE => {
                ...
                self.bdcr.modify(|_, w| w.rtcsel().bits(RtcSel::LSE as u8));
            }
            Osc::LSI => {
                ...
                self.bdcr.modify(|_, w| w.rtcsel().bits(RtcSel::LSI as u8));
            }
            Osc::HSE => {
                ...
                self.bdcr.modify(|_, w| w.rtcsel().bits(RtcSel::HSE as u8));
            }
            Osc::HSI => {
                /* Unusable clock source, here to prevent warnings. */
                /* Turn off clock sources to RTC. */
                self.bdcr
                    .modify(|_, w| w.rtcsel().bits(RtcSel::NOCLK as u8));
            }
            _ => {}
        }
    }
    ...
    fn set_adcpre(&mut self, adcpre: AdcPre) {
        self.cfgr.modify(|_, w| w.adcpre().variant(adcpre));
    }

    fn set_ppre2(&mut self, ppre2: PPre2) {
        self.cfgr.modify(|_, w| w.ppre2().variant(ppre2));
    }

    fn set_ppre1(&mut self, ppre1: PPre1) {
        self.cfgr.modify(|_, w| w.ppre1().variant(ppre1));
    }

    fn set_hpre(&mut self, hpre: HPre) {
        self.cfgr.modify(|_, w| w.hpre().variant(hpre));
    }

    fn set_usbpre(&mut self, usbpre: UsbPre) {
        self.cfgr.modify(|_, w| w.otgfspre().variant(usbpre));
    }
...
impl RccExt for RCC {
    fn peripheral_enable_clock(&mut self, en: Enr) {
        match en {
            Enr::Ahb(e) => {
                self.ahbenr
                    .modify(|r, w| unsafe { w.bits(r.bits() | e.value()) });
            }
            Enr::Apb1(e) => {
                self.apb1enr
                    .modify(|r, w| unsafe { w.bits(r.bits() | e.value()) });
    ...
    fn peripheral_reset(&mut self, reset: Enr) {
        match reset {
            Enr::Apb1(rt) => {
                self.apb1rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() | rt.value()) });
            }
            Enr::Apb2(rt) => {
                self.apb2rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() | rt.value()) });
    ...
    fn peripheral_clear_reset(&mut self, clear_reset: Enr) {
        match clear_reset {
            Enr::Apb1(cr) => {
                self.apb1rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() & !cr.value()) });
            }
            Enr::Apb2(cr) => {
                self.apb2rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() & !cr.value()) });
    ...
    fn periph_clock_enable(&mut self, clken: Per) {
        match clken {
            Per::Ahb(e) => {
                self.ahbenr
                    .modify(|r, w| unsafe { w.bits(r.bits() | (1 << e)) });
            }
            Per::Apb1(e) => {
                self.apb1enr
                    .modify(|r, w| unsafe { w.bits(r.bits() | (1 << e)) });
    ...
    fn periph_reset_pulse(&mut self, rst: Per) {
        match rst {
            Per::Apb1(rt) => {
                self.apb1rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() | (1 << rt)) });
            }
            Per::Apb2(rt) => {
                self.apb2rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() | (1 << rt)) });
    ...
    fn periph_reset_release(&mut self, rst: Per) {
        match rst {
            Per::Apb1(rs) => {
                self.apb1rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() & !(1 << rs)) });
            }
            Per::Apb2(rs) => {
                self.apb2rstr
                    .modify(|r, w| unsafe { w.bits(r.bits() & !(1 << rs)) });
    ...
    fn set_mco(&mut self, mcosrc: Mco) {
        self.cfgr
            .modify(|_, w| unsafe { w.pllmul().bits(mcosrc as u8) });
    }

    fn osc_bypass_enable(&mut self, osc: Osc) {
        match osc {
            Osc::HSE => {
                self.cr.modify(|_, w| w.hsebyp().set_bit());
            }
            Osc::LSE => {
                //let reg = &self.csr;
                let reg = &self.bdcr;
                reg.modify(|_, w| w.lsebyp().set_bit());
            }
    ...
    fn osc_bypass_disable(&mut self, osc: Osc) {
        match osc {
            Osc::HSE => {
                self.cr.modify(|_, w| w.hsebyp().clear_bit());
            }
            Osc::LSE => {
                //let reg = &self.csr;
                let reg = &self.bdcr;
                reg.modify(|_, w| w.lsebyp().clear_bit());
            }
            /* Do nothing, only HSE/LSE allowed here. */
            _ => {}
        }
