...
macro_rules! impl_adc {
    ($ADCx:ty) => {
        impl AdcExt for $ADCx {
            #[inline]
            fn power_off(&mut self) {
                self.cr2.modify(|_, w| w.adon().clear_bit());
            }

            #[inline]
            fn enable_analog_watchdog_regular(&mut self) {
                self.cr1.modify(|_, w| w.awden().set_bit());
            }

            #[inline]
            fn disable_analog_watchdog_regular(&mut self) {
                self.cr1.modify(|_, w| w.awden().clear_bit());
            }

            #[inline]
            fn enable_analog_watchdog_injected(&mut self) {
                self.cr1.modify(|_, w| w.jawden().set_bit());
            }

            #[inline]
            fn disable_analog_watchdog_injected(&mut self) {
                self.cr1.modify(|_, w| w.jawden().clear_bit());
            }

            #[inline]
            fn enable_discontinuous_mode_regular(&mut self, length: u8) {
                assert!((length - 1) <= 7);
                self.cr1.modify(|_, w| w.discen().set_bit());
                self.cr1
                    .modify(|_, w| unsafe { w.discnum().bits(length - 1) });
            }

            #[inline]
            fn disable_discontinuous_mode_regular(&mut self) {
                self.cr1.modify(|_, w| w.discen().clear_bit());
            }

            #[inline]
            fn enable_discontinuous_mode_injected(&mut self) {
                self.cr1.modify(|_, w| w.jdiscen().set_bit());
            }

            #[inline]
            fn disable_discontinuous_mode_injected(&mut self) {
                self.cr1.modify(|_, w| w.jdiscen().clear_bit());
            }

            #[inline]
            fn enable_automatic_injected_group_conversion(&mut self) {
                self.disable_external_trigger_injected();
                self.cr1.modify(|_, w| w.jauto().set_bit());
            }

            #[inline]
            fn disable_automatic_injected_group_conversion(&mut self) {
                self.cr1.modify(|_, w| w.jauto().clear_bit());
            }

            #[inline]
            fn enable_analog_watchdog_on_all_channels(&mut self) {
                self.cr1.modify(|_, w| w.awdsgl().clear_bit());
            }
            ...
            fn enable_analog_watchdog_on_selected_channel(&mut self, channel: Channel) {
                ...
                assert!(ch <= AWDCH_MAX);
                self.cr1
                    .modify(|_, w| unsafe { w.awdch().bits(ch).awdsgl().set_bit() });
            }

            #[inline]
            fn enable_scan_mode(&mut self) {
                self.cr1.modify(|_, w| w.scan().set_bit());
            }

            #[inline]
            fn disable_scan_mode(&mut self) {
                self.cr1.modify(|_, w| w.scan().clear_bit());
            }

            #[inline]
            fn enable_eoc_interrupt_injected(&mut self) {
                self.cr1.modify(|_, w| w.jeocie().set_bit());
            }

            #[inline]
            fn disable_eoc_interrupt_injected(&mut self) {
                self.cr1.modify(|_, w| w.jeocie().clear_bit());
            }

            #[inline]
            fn enable_awd_interrupt(&mut self) {
                self.cr1.modify(|_, w| w.awdie().set_bit());
            }

            #[inline]
            fn disable_awd_interrupt(&mut self) {
                self.cr1.modify(|_, w| w.awdie().clear_bit());
            }
            ...
            #[inline]
            fn set_left_aligned(&mut self) {
                self.cr2.modify(|_, w| w.align().set_bit());
            }

            #[inline]
            fn set_right_aligned(&mut self) {
                self.cr2.modify(|_, w| w.align().clear_bit());
            }

            #[inline]
            fn eoc(&self) -> bool {
                self.sr.read().eoc().bit_is_set()
            }

            #[inline]
            fn eoc_injected(&self) -> bool {
                self.sr.read().jeoc().bit_is_set()
            }
            ...
            #[inline]
            fn set_single_conversion_mode(&mut self) {
                self.cr2.modify(|_, w| w.cont().clear_bit());
            }

            #[inline]
            fn set_watchdog_high_threshold(&mut self, threshold: u16) {
                self.htr
                    .write(|w| unsafe { w.bits((threshold as u32) & HT_MSK) });
            }

            #[inline]
            fn set_watchdog_low_threshold(&mut self, threshold: u16) {
                self.ltr
                    .write(|w| unsafe { w.bits((threshold as u32) & LT_MSK) });
            }
            ...
            fn set_regular_sequence(&mut self, channels: &[Channel]) {
                ...
                self.sqr1
                    .write(|w| unsafe { w.bits(third6 | (((length - 1) as u32) << SQR1_L_LSB)) });
                self.sqr2.write(|w| unsafe { w.bits(second6) });
                self.sqr3.write(|w| unsafe { w.bits(first6) });
                //}
            }
            ...
            fn set_injected_sequence(&mut self, channels: &[Channel]) {
                ...
                self.jsqr.write(|w| unsafe { w.bits(reg32) });
            }
            ...
            fn set_injected_offset(&mut self, reg: InjectedChannel, offset: u32) {
                ...
                jofr.write(|w| unsafe { w.bits(offset) });
            }

            #[inline]
            fn start_conversion_regular(&mut self) {
                // Start conversion on regular channels.
                self.cr2.modify(|_, w| w.swstart().set_bit());
            ...
            fn start_conversion_injected(&mut self) {
                ...
                self.cr2.modify(|_, w| w.jswstart().set_bit());
                // Wait until the ADC starts the conversion.
            ...
            #[inline]
            fn enable_dma(&mut self) {
                self.cr2.modify(|_, w| w.dma().set_bit());
            }

            #[inline]
            fn disable_dma(&mut self) {
                self.cr2.modify(|_, w| w.dma().clear_bit());
            }
        }
        ...
        impl AdcSpecialExt for $ADCx {
            ...
            #[inline]
            fn disable_external_trigger_regular(&mut self) {
                self.cr2.modify(|_, w| w.exttrig().clear_bit());
            }

            #[inline]
            fn disable_external_trigger_injected(&mut self) {
                self.cr2.modify(|_, w| w.jexttrig().clear_bit());
            }
            ...
            #[inline]
            fn set_sample_time_on_all_channels(&mut self, time: SampleTime) {
                let reg32 = (0..10).fold(0u32, |c, i| c | ((time as u32) << (i * 3)));
                self.smpr2.write(|w| unsafe { w.bits(reg32) });
                let reg32 = (10..18).fold(0u32, |c, i| c | (u32::from(time) << ((i - 10) * 3)));
                self.smpr1.write(|w| unsafe { w.bits(reg32) });
            }
        }
    };
...
macro_rules! impl_adc_trigger12 {
    ($ADCx:ty) => {
        impl AdcTrigger for $ADCx {
            ...
            fn enable_external_trigger_regular(&mut self, trigger: ExternalTrigger) {
                self.cr2
                    .modify(|_, w| unsafe { w.extsel().bits(trigger.into()).exttrig().set_bit() });
            }

            #[inline]
            fn enable_external_trigger_injected(&mut self, trigger: ExternalTriggerInjected) {
                self.cr2.modify(|_, w| unsafe {
                    w.jextsel().bits(trigger.into()).jexttrig().set_bit()
                });
            }
        }
    };
...
impl AdcDualExt for ADC1 {
    ...
    fn set_dual_mode(&mut self, mode: DualMode) {
        self.cr1
            .modify(|_, w| unsafe { w.dualmod().bits(mode.into()) });
    }
}
...
impl AdcTemperatureExt for ADC1 {
    #[inline]
    fn enable_temperature_sensor(&mut self) {
        self.cr2.modify(|_, w| w.tsvrefe().set_bit());
    }

    #[inline]
    fn disable_temperature_sensor(&mut self) {
        self.cr2.modify(|_, w| w.tsvrefe().clear_bit());
    }
}
