...
impl FlashExt for FLASH {
    fn prefetch_buffer_enable(&mut self) {
        self.acr.modify(|_, w| w.prftbe().set_bit());
    }

    fn prefetch_buffer_disable(&mut self) {
        self.acr.modify(|_, w| w.prftbe().clear_bit());
    }

    fn set_ws(&mut self, ws: FlashLatency) {
        self.acr.modify(|_, w| w.latency().variant(ws));
    }
    ...
    fn unlock(&mut self) {
        ...
        self.keyr.write(|w| unsafe { w.bits(FLASH_KEYR_KEY1) });
        self.keyr.write(|w| unsafe { w.bits(FLASH_KEYR_KEY2) });
    }
    ...
    fn clear_pgerr_flag(&mut self) {
        self.sr.modify(|_, w| w.pgerr().set_bit());
    }

    fn clear_eop_flag(&mut self) {
        self.sr.modify(|_, w| w.eop().set_bit());
    }

    fn clear_wrprterr_flag(&mut self) {
        self.sr.modify(|_, w| w.wrprterr().set_bit());
    }
    /*
    ...
    */
    ...
    fn unlock_option_bytes(&mut self) {
        ...
        self.optkeyr.write(|w| unsafe { w.bits(FLASH_KEYR_KEY1) });
        self.optkeyr.write(|w| unsafe { w.bits(FLASH_KEYR_KEY2) });
    }
    ...
    fn erase_option_bytes(&mut self) {
        ...
        self.cr.modify(|_, w| {
            w.opter()
                .set_bit() /* Enable option byte erase. */
                .strt()
                .set_bit()
        });
        self.wait_for_last_operation();
        self.cr.modify(
            |_, w| w.opter().clear_bit(), /* Disable option byte erase. */
        );
    }
    ...
    fn program_option_bytes(&mut self, address: u32, data: u16) {
        ...
        self.cr.modify(
            |_, w| w.optpg().set_bit(), /* Enable option byte programming. */
        );
        ...
        self.wait_for_last_operation();
        self.cr.modify(
            |_, w| w.optpg().clear_bit(), /* Disable option byte programming. */
        );
    }
}
...
impl FlashF1Ext for FLASH {
    fn halfcycle_enable(&mut self) {
        self.acr.modify(|_, w| w.hlfcya().set_bit());
    }

    fn halfcycle_disable(&mut self) {
        self.acr.modify(|_, w| w.hlfcya().clear_bit());
    }
}
...
impl FlashF1LowHighExt for FLASH {
    ...
    */
    ...
    fn erase_page(&mut self, page_address: u32) {
        ...
        self.ar.write(|w| unsafe { w.bits(page_address) });
        self.cr.modify(|_, w| w.strt().set_bit());
    ...
    fn erase_all_pages(&mut self) {
        ...
        self.cr
            .modify(|_, w| w.mer().set_bit() /* Enable mass erase. */);
        self.cr
            .modify(|_, w| w.strt().set_bit() /* Trigger the erase. */);

        self.wait_for_last_operation();
        self.cr
            .modify(|_, w| w.mer().clear_bit() /* Disable mass erase. */);
    }
}
...
impl FlashF1LowHighExt for FLASH {
    ...
    fn program_half_word(&mut self, address: u32, data: u16) {
        ...
        if address >= FLASH_BASE + 0x00080000 {
            self.cr2.modify(|_, w| w.pg().set_bit());
        } else {
            self.cr.modify(|_, w| w.pg().set_bit());
        }

        unsafe {
            ptr::write_volatile(*address, data as u32);
        }

        self.wait_for_last_operation();

        if address >= FLASH_BASE + 0x00080000 {
            self.cr2.modify(|_, w| w.pg().clear_bit());
        } else {
            self.cr.modify(|_, w| w.pg().clear_bit());
        }
    }
    ...
    fn erase_page(&mut self, page_address: u32) {
        ...
        if page_address >= FLASH_BASE + 0x00080000 {
            self.cr2.modify(|_, w| w.per().set_bit());
            self.ar2.write(|w| w.bits(page_address));
            self.cr2.modify(|_, w| w.strt().set_bit());
        } else {
            self.cr.modify(|_, w| w.per().set_bit());
            self.ar.write(|w| w.bits(page_address));
            self.cr.modify(|_, w| w.strt().set_bit());
        }
        ...
        if page_address >= FLASH_BASE + 0x00080000 {
            self.cr2.modify(|_, w| w.per().clear_bit());
        } else {
            self.cr.modify(|_, w| w.per().clear_bit());
        }
    }
    ...
    fn erase_all_pages(&mut self) {
        ...
        /* Repeat for bank 2 */
        self.cr2.modify(|_, w| w.mer().set_bit());
        self.cr2.modify(|_, w| w.strt().set_bit());

        self.wait_for_last_operation();
        self.cr2.modify(|_, w| w.mer().clear_bit());
    }
}
...
impl FlashF1HighExt for FLASH {
    fn unlock_upper(&mut self) {
        /* Clear the unlock state. */
        self.cr2.modify(|_, w| w.lock().clear_bit());

        /* Authorize the FPEC access. */
        self.keyr2.write(|w| w.bits(FLASH_KEYR_KEY1));
        self.keyr2.write(|w| w.bits(FLASH_KEYR_KEY2));
    }

    fn lock_upper(&mut self) {
        self.cr2.modify(|_, w| w.lock().set_bit());
    }

    fn clear_pgerr_flag_upper(&mut self) {
        self.sr2.modify(|_, w| w.pgerr().set_bit());
    }

    fn clear_eop_flag_upper(&mut self) {
        self.sr2.modify(|_, w| w.eop().set_bit());
    }

    fn clear_wrprterr_flag_upper(&mut self) {
        self.sr2.modify(|_, w| w.wrprterr().set_bit());
    }

    fn clear_bsy_flag_upper(&mut self) {
        self.sr2.modify(|_, w| w.bsy().clear_bit());
    }
}
